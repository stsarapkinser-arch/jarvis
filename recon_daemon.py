...
class ReconDaemon(metaclass=Singleton):
    ...
    async def watch_auth_log(self) -> None:
        """Tail auth events. Prefers /var/log/auth.log (via ``tail -F``, which
        uses inotify under the hood — no polling), falls back to
        ``journalctl -f`` filtered on sshd/sudo/su when systemd-only systems
        (modern Kali, Fedora) don't ship that file."""
        path = next((p for p in AUTH_LOG_CANDIDATES if os.path.exists(p)), None)
        if path is not None:
            await self._tail_authlog(path)
            return
        if shutil.which("journalctl"):
            log.info("recon: no auth.log file; using journalctl -f fallback")
            await self._tail_auth_journal()
            return
    ...
    async def _tail_auth_journal(self) -> None:
        """journalctl --follow on the auth-relevant unit/identifiers. journald
        wakes us via netlink — no polling, just a blocking read on stdout."""
        cmd = (
            "journalctl", "-f", "--no-pager", "-n", "0", "--output=short",
        ...
        if proc is None or proc.stdout is None:
            return
        log.info("recon: auth journal watcher started")
        try:
            while not self._stopped:
                raw = await proc.stdout.readline()
                if not raw:
