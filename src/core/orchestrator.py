...
from src.core.ladder import RecognitionLadderMixin
from src.core.loops import BackgroundLoopsMixin
# Логика подтверждения вынесена в src/core/confirm.py — её разделяют оркестратор
# и mixin лестницы. Реэкспорт сохраняет обратную совместимость внешних импортов
# (is_confirm_phrase и пр.) из этого модуля.
from src.core.confirm import (
    AFFIRMATIVE_RE,
    CONFIRM_HINT,
    NEGATIVE_RE,
    is_confirm_phrase,
...
class Jarvis(RecognitionLadderMixin, BackgroundLoopsMixin, metaclass=Singleton):
