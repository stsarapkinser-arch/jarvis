...
DEFAULT_FRAME_MS = 15   # был 40 ms — урезан до 15 ms для нулевой задержки пульса
DEFAULT_BANDS = 24
...
def smooth_bands(prev: Sequence[float], curr: Sequence[float], alpha: float = 0.70) -> tuple[float, ...]:
    """Exponential smoothing so the HUD doesn't strobe.

    alpha=0.70: преобладает текущий кадр — пульс следует голосу почти мгновенно
    (задержка ~1 кадр = 15 ms). Было 0.55 (2 кадра задержки)."""
...
class PiperFFTPump(metaclass=Singleton):
    ...
    DEFAULT_PUMP_CHUNK = 512  # был 1024 (~46ms) → 512 (~23ms) — вдвое быстрее реакция
    ...
    def _publish(self, frame: FFTFrame) -> None:
        # Cap publish rate at ~80 Hz — HUD repaints 30 fps, но мы публикуем чаще
        # чтобы ни один всплеск голоса не пропал между двумя кадрами HUD.
        # Было 0.018 s (55 Hz); теперь 0.010 s (100 Hz) — задержка пульса = 0.
        if frame.ts - self._last_publish_ts < 0.010:
            return
